...
if ($op=="suicide" && getsetting("selfdelete",0)!=0) {
    ...
    // Set defaults to prevent undefined key warnings
    ...
    $form=array(
        ...
        "timeformat"=>array(
            "Timestamp format (currently displaying time as %s whereas default format is \"[m/d h:ia]\"),string,20",
            date(
        ...
        "timeoffset"=>array(
            "Hours to offset time displays (%s currently displays as %s)?,int",
            date($session['user']['prefs']['timeformat'] ?? "[m/d h:ia]"),
            date(
                $session['user']['prefs']['timeformat'] ?? "[m/d h:ia]",
                strtotime("now") + (($session['user']['prefs']['timeoffset'] ?? 0) * 60 * 60)
            )
