	...
	foreach ($navbysection as $key => $val) {
		...
		if (count_viable_navs($key) > 0) {
			if ($key > '') {
				...
				// --- Defensive: format nav section as string if it's an array ---
				if (is_array($key)) {
					// Use vsprintf if possible, or implode for safety
					if (count($key) > 1 && is_string($key[0])) {
						$key = vsprintf($key[0], array_slice($key, 1));
					} else {
						$key = implode(" ", $key);
					}
				}
			...
			foreach ($val as $k => $v) {
				if (is_array($v) && count($v)>0){
					unset($v['translate']);
					// --- Defensive: format nav label as string if it's an array ---
					if (is_array($v[0])) {
						if (count($v[0]) > 1 && is_string($v[0][0])) {
							$v[0] = vsprintf($v[0][0], array_slice($v[0], 1));
						} else {
							$v[0] = implode(" ", $v[0]);
						}
					}
...
function private_addnav($text,$link=false,$priv=false,$pop=false,$popsize="500x300"){
	...
	// --- Defensive: convert array label to string for all navs ---
	if (is_array($text)){
		// If the nav label is itself an array, format it as a string
		if (count($text) > 1 && is_string($text[0])) {
			$text = vsprintf($text[0], array_slice($text, 1));
		} else {
			$text = implode(" ", $text);
		}
	}
	...
	// Defensive: Only access navschema if key is set
