...
if ($replyTo) {
    $sql = db_query(
        ...
        WHERE msgto = '{$session['user']['acctid']}' AND messageid = '" . addslashes($replyTo) . "'"
    );
...
if ($to) {
    // Try to find by login or name, case-insensitive, strip color codes from name
    $toEsc = addslashes($to);
    $sql = db_query(
        "SELECT login, name, superuser FROM $accounts
        WHERE LOWER(login) = LOWER('$toEsc') OR LOWER(name) = LOWER('$toEsc')"
    );
...
if (isset($row['login']) && $row['login']!="") {
    ...
    $to = implode('%', $to);
    $to = "%$to%";
    // Defensive: always use db_prefix, and case-insensitive LIKE
    $sql = db_query(
        "SELECT login, name, superuser FROM $accounts
        WHERE (LOWER(login) LIKE LOWER('$to') OR LOWER(name) LIKE LOWER('$to'))
        AND locked = 0
    ...
    while ($row = db_fetch_assoc($sql)) {
        $rowNum++;
        if ($numRows == 1) {
            rawoutput("<input type='hidden' name='to' id='to' value='" . htmlent($row['login']) . "'>");
            output_notl(htmlent($row['name']) . "`0`n");
        }
        else {
            output_notl(
