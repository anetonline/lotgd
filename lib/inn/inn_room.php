<?php
// Defensive unserialize for donationconfig - prevents unserialize errors if data is not a valid serialized string
$donationconfig_raw = $session['user']['donationconfig'] ?? "";
if ($donationconfig_raw === "0" || $donationconfig_raw === "" || $donationconfig_raw === null) {
    $config = [];
} else {
    $config = @unserialize($donationconfig_raw);
...
$pay = httpget('pay');
if ($pay){
    if ($pay == 2 || $session['user']['gold'] >= $expense || $session['user']['boughtroomtoday']){
        if ($session['user']['loggedin']){
            if (!$session['user']['boughtroomtoday']) {
                if ($pay == 2) {
                    $fee = getsetting("innfee", "5%");
                    if (strpos($fee, "%") !== false) {
                        $percent = floatval(str_replace('%', '', $fee));
                        $expense += round($expense * $percent / 100, 0);
                    } else {
                        // Defensive: ensure $fee is numeric
                        $expense += is_numeric($fee) ? floatval($fee) : 0;
                    }
    ...
    if ($session['user']['boughtroomtoday']){
        ...
        if (strpos($fee, "%") !== false) {
            $percent = floatval(str_replace('%', '', $fee));
            $bankexpense = $expense + round($expense * $percent / 100, 0);
        } else {
            $bankexpense = $expense + (is_numeric($fee) ? floatval($fee) : 0);
        }
