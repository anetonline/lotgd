...
/**
 * Safe unserialize function that ignores extra data after the first structure.
 */
function safe_unserialize_first($data) {
    if (!is_string($data)) return false;
    $data = trim($data);

    // Try to match a serialized array or object from the start (ungreedy)
    if (preg_match('/^(a|O):\d+:\{.*\}$/s', $data)) {
        // Looks fine, unserialize as is
        return unserialize($data);
    } elseif (preg_match('/^(a|O):\d+:\{.*\}/sU', $data, $matches)) {
        // Ungreedy: shortest possible valid string
        return unserialize($matches[0]);
    } else {
        // Fallback: find the last closing brace
        $pos = strrpos($data, '}');
        if ($pos !== false) {
            $short = substr($data, 0, $pos + 1);
            return unserialize($short);
        }
    }
    return false;
}
...
<input type="image" src="images/logdnet.php" border="0" width="62" height="57" name="submit" alt="Donate!">
	...
	foreach ($servers as $key => $val) {
		// --- FIX: Use safe unserialize to avoid warnings about extra data ---
		$row = safe_unserialize_first($val);
